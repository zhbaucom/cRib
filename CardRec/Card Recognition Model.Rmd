...
test_lab <- all_label[Nummy] %>%
  as.factor()  %>%
  as.integer() %>%
  minus1() %>%
  to_categorical()
test_y <- all_label[Nummy] %>%
  as.factor()  %>%
  as.integer() %>%
  minus1() 
...
train_lab <- all_label[!Nummy]%>%
  as.factor()  %>%
  as.integer() %>%
  minus1() %>%
  to_categorical()
train_y <- all_label[!Nummy]%>%
  as.factor()  %>%
  as.integer() %>%
  minus1()
...
#----------------------------(D)-----------------------------------#
layer_conv_2d(filters = 52*20,      #-----------Third convoluted layer
 kernel_size = c(3,3),            #----80 Filters with dimension 4x4
 activation = "relu") %>%         #--with a ReLu activation function
#-----------------------------(E)----------------------------------#
layer_conv_2d(filters = 52*20,      #----------Fourth convoluted layer
 kernel_size = c(3,3),            #----80 Filters with dimension 4x4
 activation = "relu") %>%         #--with a ReLu activation function
#-----------------------------(F)----------------------------------#
layer_max_pooling_2d(pool_size = c(3,3)) %>%  #---------Max Pooling
...
#------------------------------(H)---------------------------------#
layer_dense(units = 52*40, activation = "relu")%>% #-----Hidden layer
#---------------------------(I)-----------------------------------#
layer_dropout(rate= 0.25)%>%     #-------------------Drop-out layer
#-----------------------------------------------------------------#
